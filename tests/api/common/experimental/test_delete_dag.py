...
class TestDeleteDAGCatchError(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.subdag_id = 'example_subdag_operator.section-1'
        self.subdag = self.dagbag.dags[self.subdag_id]
    ...
    def test_delete_subdag_subdag_still_in_dagbag(self):
        with create_session() as session:
            models_to_check = ['DagModel', 'DagRun', 'TaskInstance']
            record_counts = {}
            ...
            for model_name in models_to_check:
                m = getattr(models, model_name)
                record_counts[model_name] = session.query(m).filter(m.dag_id == self.subdag_id).count()

            with self.assertRaises(DagFileExists):
                delete_dag(self.subdag_id)
...
class TestDeleteSubdagSuccessfulDelete(unittest.TestCase):

    def setUp(self):
        self.key = "test_dag_id.subdag"
        ...
        d = days_ago(1)
        with create_session() as session:
            session.add(DM(dag_id=self.key, is_subdag=True))
            session.add(DR(dag_id=self.key))
            session.add(TI(task=task,
                           execution_date=d,
                           state=State.SUCCESS))
